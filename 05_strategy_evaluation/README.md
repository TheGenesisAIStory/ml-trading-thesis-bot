...
The original examples relied on legacy `zipline`, `pyfolio`, `pandas_datareader`, and symbolic dependencies that are fragile on current Python versions. The notebooks in this folder now use [`strategy_evaluation_utils.py`](strategy_evaluation_utils.py) for deterministic local data, vectorized portfolio calculations, and portable performance metrics.

| Notebook | Modern focus |
|---|---|
| [`01_backtest_with_trades.ipynb`](01_backtest_with_trades.ipynb) | Vectorized trade/backtest preview with moving-average signals |
| [`02_backtest_with_pf_optimization.ipynb`](02_backtest_with_pf_optimization.ipynb) | Backtest plus mean-variance portfolio weighting |
| [`03_pyfolio_demo.ipynb`](03_pyfolio_demo.ipynb) | Pyfolio-style metrics without the deprecated pyfolio dependency |
| [`04_mean_variance_optimization.ipynb`](04_mean_variance_optimization.ipynb) | Mean-variance weights and portfolio diagnostics |
| [`05_kelly_rule.ipynb`](05_kelly_rule.ipynb) | Kelly sizing with empirical win-rate inputs |

Recommended command:

```bash
cd 05_strategy_evaluation
for nb in *.ipynb; do python3.11 -m nbconvert --to notebook --execute --inplace "$nb"; done
```

...
The notebooks now use [`time_series_utils.py`](time_series_utils.py) for deterministic local time-series examples. This removes runtime dependence on `pandas_datareader`, `statsmodels`, `arch`, and `pykalman` while preserving the chapter concepts.

| Notebook | Modern focus |
|---|---|
| `01_tsa_and_stationarity.ipynb` | Stationarity diagnostics |
| `02_arima_models.ipynb` | AR-style forecasting with numpy regression |
| `03_arch_garch_models.ipynb` | EWMA/GARCH-like volatility proxy |
| `04_vector_autoregressive_model.ipynb` | VAR-style multivariate regression |
| `05_cointegration_tests.ipynb` | Hedge ratio and spread diagnostics |
| `06_statistical_arbitrage_with_cointegrated_pairs.ipynb` | Pair spread and z-score signals |
| `07_pairs_trading_backtest.ipynb` | Vectorized pairs trading backtest |

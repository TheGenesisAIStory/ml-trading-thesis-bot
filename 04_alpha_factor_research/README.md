...
## Modern Alpha Factor Workflow

The modernized local workflow adds reproducible fallbacks and Parquet outputs so research notebooks can run without legacy Zipline/Alphalens dependencies.

| File | Purpose |
|---|---|
| [`alpha_factor_utils.py`](alpha_factor_utils.py) | Shared data loading, synthetic fallback, factor engineering, rank IC, walk-forward splits, and portfolio metrics |
| [`00_alpha_factor_research_definitive.ipynb`](00_alpha_factor_research_definitive.ipynb) | Definitive factor panel and diagnostics entry point |
| [`01_ml_walk_forward_factor_backtest_definitive.ipynb`](01_ml_walk_forward_factor_backtest_definitive.ipynb) | Walk-forward ML factor model and long/short backtest |
| [`02_how_to_use_talib.ipynb`](02_how_to_use_talib.ipynb) | Technical indicators, with pandas fallback when TA-Lib is unavailable |
| [`requirements.txt`](requirements.txt) | Minimal modern dependencies for this folder |

Recommended command:

```bash
cd 04_alpha_factor_research
python3.11 -m nbconvert --to notebook --execute --inplace 01_ml_walk_forward_factor_backtest_definitive.ipynb
```
...
The notebook [01_feature_engineering](01_feature_engineering.ipynb) illustrates how to engineer basic factors.
...
The definitive notebook [01_ml_walk_forward_factor_backtest_definitive](01_ml_walk_forward_factor_backtest_definitive.ipynb) illustrates how to combine engineered factors in a walk-forward ML workflow. Legacy Quantopian research notebooks are intentionally not required for local execution.

...
The maintained notebooks run on Python 3.10+ using `scikit-learn` gradient boosting models. Optional libraries from the original chapter (`LightGBM`, `CatBoost`, `SHAP`, `Zipline`) are replaced by portable equivalents: histogram gradient boosting, permutation importance, partial dependence, quantile diagnostics, and vectorized backtests.

Artifacts are written to `../data/gradient_boosting/`.

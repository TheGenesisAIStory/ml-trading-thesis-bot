...
The original chapter used PyMC3, Theano, and remote macro data readers. The maintained notebooks now run on Python 3.10+ without mandatory probabilistic-programming backends. They use conjugate Bayesian updates, posterior simulation, rolling regression, and volatility proxies so the chapter remains reproducible offline.

| Notebook | Modern output |
| --- | --- |
| `01_updating_conjugate_priors.ipynb` | Beta-binomial posterior updates for market direction probabilities. |
| `02_pymc3_workflow.ipynb` | PyMC-style Bayesian workflow using analytic normal posteriors and posterior samples. |
| `03_bayesian_sharpe_ratio.ipynb` | Posterior Sharpe ratio comparison for synthetic strategies. |
| `04_rolling_regression.ipynb` | Rolling hedge-ratio estimation for dynamic factor exposure. |
| `05_stochastic_volatility.ipynb` | Latent-volatility proxy based on exponentially weighted return variance. |

Artifacts are written to `../data/bayesian_machine_learning/` and are intentionally excluded from git when they are generated data.

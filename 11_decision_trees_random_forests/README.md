...
The maintained notebooks run on Python 3.10+ with `scikit-learn` and a reproducible synthetic equity panel. Legacy integrations that are often unavailable on modern systems (`LightGBM`, `Alphalens`, `Zipline`) are represented by local equivalents: signal diagnostics, feature importance, and vectorized long-short backtests.

| Notebook | Modern output |
| --- | --- |
| `00_data_prep.ipynb` | Synthetic multi-asset feature panel saved as Parquet. |
| `01_decision_trees.ipynb` | Regression/classification tree baseline and feature importance. |
| `02_bagged_decision_trees.ipynb` | Bagging versus single-tree error comparison. |
| `03_random_forest_tuning.ipynb` | Time-series cross-validation for random forest hyperparameters. |
| `04_japanese_equity_features.ipynb` | Cross-sectional feature diagnostics compatible with the old workflow. |
| `05_random_forest_return_signals.ipynb` | Random-forest probability scores for return direction. |
| `06_alphalens_signals_quality.ipynb` | Quantile-based signal quality summary without Alphalens dependency. |
| `07_backtesting_with_zipline.ipynb` | Vectorized long-short backtest without Zipline runtime. |

Artifacts are written to `../data/tree_models/`.
